  ...
  .form-label-group input:-ms-input-placeholder {
    ...
    <body style="background: linear-gradient(to right, #6b6b6b, #612103);" class="sb-nav-fixed">
        ...
        <div id="layoutSidenav">
            ...
            <?php
                $query="SELECT * FROM agregados_postres WHERE id_agregado_postre=$id_detalle";
                $resultado=$mysqli->query($query);
                if($resultado->num_rows > 0){
                    $fila=$resultado->fetch_assoc();
                    $id_pizza=$fila['id_agregado_postre'];
                    $nombre=$fila['nombre_agregado_postre'];
                    $precio=$fila['precio_agregado_postre'];
                    $caracteristicas=$fila['caracteristica_agregado_postre'];
            ?>
            <!-- Parte del centro -->
