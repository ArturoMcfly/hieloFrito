  ...
  .form-label-group input:-ms-input-placeholder {
    ...
    <body style="background: linear-gradient(to right, #6b6b6b, #612103);" class="sb-nav-fixed">
        ...
        <div id="layoutSidenav">
            ...
            <?php
                $query="SELECT * FROM agregados_entradas WHERE id_agregado_entradas=$id_detalle";
                $resultado=$mysqli->query($query);
                if($resultado->num_rows > 0){
                    $fila=$resultado->fetch_assoc();
                    $id_pizza=$fila['id_agregado_entradas'];
                    $nombre=$fila['nombre_agregado_entrada'];
                    $precio=$fila['precio_agregado_entrada'];
                    $caracteristicas=$fila['caracteristica_agregado_entrada'];
            ?>
            <!-- Parte del centro -->
